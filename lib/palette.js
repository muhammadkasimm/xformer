 ...
 */
const pickByRegex = exports.pickByRegex = R.curry((text, data) => R.ifElse(R.always(isNothing(text)), R.always({}), R.pickBy((v, k) => R.test(new RegExp(text), k)))(data));
 ...
 * @returns {Object}
 */
const mergeWithOp = R.curry((xformer, data) => R.ifElse(_.typeMatches('array'), R.pipe(R.reject(isNothing), R.reduce(R.mergeDeepWith(xformer), {})), R.pipe(R.pickBy(_.typeMatches('object')), R.values, R.reduce(R.mergeDeepWith(xformer), {})))(data));
 ...
 *        //=> {a:1,b:2}
 */
const cleanDataByKeys = exports.cleanDataByKeys = R.curry((predicates, data) => {
  return R.cond([[_.typeMatches('array'), R.pipe(R.pickBy((v, k) => R.and(_.isSomething(k), R.complement(R.anyPass(D.decodePipe(predicates)))(k))), R.values)], [_.typeMatches('object'), R.pickBy((v, k) => R.and(_.isSomething(k), R.complement(R.anyPass(D.decodePipe(predicates)))(k)))]])(data);
});
