...
const differential = exports.differential = R.memoizeWith(R.toString, R.curry(data => {
  const calculator = R.pipe(R.toPairs, R.reject(x => H.isJunk(x[1])), R.sortBy(R.prop(0)), R.aperture(2), R.map(([prev, next]) => [next[0], R.subtract(...R.pluck(1, [next, prev]))]), R.map(R.over(R.lensIndex(1), R.pipe(H.defaultToZero, H.bePositive))), R.fromPairs);
