 ...
 * @returns {boolean}
 */
const divideBy = exports.divideBy = R.curry((denominator, numerator) => {
  return R.pipe(defaultTo(1), R.divide(defaultToZero(numerator)))(denominator);
});
 ...
 * @returns {any}
 */
const pickProp = exports.pickProp = R.curry((prop, data) => R.ifElse(R.always(R.equals('*', prop)), R.cond([[typeMatches('object'), R.values], [typeMatches('array'), R.pipe(R.flatten, R.map(R.values))]]), R.cond([[typeMatches('object'), R.prop(prop)], [typeMatches('array'), R.pipe(R.flatten, mapIndexed((x, i) => R.ifElse(R.always(R.pipe(parseInt, R.equals(i))(prop)), R.identity, R.prop(prop))(x)), R.reject(isNothing))]]))(data));
 ...
 * @returns {any}
 */
function pickFrom(path, data) {
  if (R.isEmpty(path)) {
    return R.when(R.allPass([typeMatches('array'), R.pipe(R.length, R.equals(1))]), R.head)(data);
  }
