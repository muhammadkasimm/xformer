...
const updateAccumulator = R.curry((fn, info, acc) => {
  return R.pipe(R.over(R.lensProp('buffer'), R.assoc(info.name, executeAction(fn, acc.result))), R.set(R.lensProp('result'), executeAction(fn, acc.result)))(acc);
});
