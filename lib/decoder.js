...
exports.getActionInfo = exports.getActionName = exports.decodePipe = exports.decodeAction = exports.translateObjectToString = exports.decodeObjectAction = exports.decodeStringyAction = exports.evaluate = undefined;
...
const evaluate = exports.evaluate = R.curry(str => {
...
const decodeStringyAction = exports.decodeStringyAction = R.curry(action => {
  return R.pipe(R.juxt([H.getFirstMatch(_constants.ALIAS_REGEX), R.pipe(R.replace(_constants.ALIAS_REGEX, ''), R.replace(_constants.OPENING_PARAN_REGEX, ''), R.replace(_constants.CLOSING_PARAN_REGEX, ''), x => `[${x}]`, eval, R.map(evaluate))]), R.ifElse(R.propSatisfies(R.has(R.__, P), 0), R.ifElse(R.propSatisfies(H.isSomething, 1), ([a, p]) => P[a](...p), ([a, p]) => P[a]), R.identity))(action);
});
...
// with the parser.
const decodeObjectAction = exports.decodeObjectAction = R.curry(action => {
  const evaluateParams = R.map(evaluate);
  const params = R.pipe(R.propOr([], 'params'), R.when(R.complement(H.typeMatches('array')), p => [p]))(action);

  return R.cond([[R.propSatisfies(R.has(R.__, P), 'name'), ({ name }) => {
    return H.isSomething(params) ? P[name](...evaluateParams(params)) : P[name];
  }], [R.allPass([R.has('fn'), R.propSatisfies(H.typeMatches('function'), 'fn')]), ({ name, fn }) => H.isSomething(params) ? fn(...evaluateParams(params)) : fn], [R.T, R.identity]])(action);
});
 ...
 */
const translateObjectToString = exports.translateObjectToString = R.curry(action => {
  return R.converge(R.concat, [R.propOr('_anonymousFn_', 'name'), R.ifElse(R.propSatisfies(H.isSomething, 'params'), R.pipe(R.prop('params'), R.map(R.toString), R.join(', '), x => `(${x})`), R.always(''))])(action);
});
 ...
 */
const decodeAction = exports.decodeAction = R.curry(action => {
  try {
 ...
 * it was called with. Command pallete is located in src/XFormer/xformerFns.
 */
const decodePipe = exports.decodePipe = R.curry(pipe => {
 ...
 */
const getActionName = exports.getActionName = R.curry(action => {
 ...
 * Takes a string or Object representation of an action and returns the name of the action.
 */
const getActionInfo = exports.getActionInfo = R.curry(action => {
