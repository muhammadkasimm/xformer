...
function decodeStringyAction(action) {
  return R.pipe(R.juxt([_.getFirstMatch(ALIAS_REGEX), R.pipe(R.replace(ALIAS_REGEX, ''), R.replace(OPENING_PARAN_REGEX, ''), R.replace(CLOSING_PARAN_REGEX, ''), x => `[${x}]`, eval, R.map(evaluate))]), R.ifElse(R.pipe(R.prop(0), R.has(R.__, P)), R.ifElse(R.propSatisfies(_.isSomething, 1), ([a, p]) => P[a](...p), ([a, p]) => P[a]), R.identity))(action);
}
