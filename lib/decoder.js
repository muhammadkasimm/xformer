...
function evaluate(str) {
  ...
  return R.pipe(R.tryCatch(eval, R.always(str)), R.when(R.test(/^[\$.]/), R.pipe(R.replace(/^[\$]\./, ''), R.prop(R.__, context))), R.when(R.test(/^[X.]/), R.pipe(R.split('.'), R.last, decodeStringyAction)), R.when(R.both(_.typeMatches('object'), R.propSatisfies(R.anyPass([R.has(R.__, P), R.has(R.__, _)]), 'name')), decodeObjectAction))(str);
}
...
function decodeStringyAction(action) {
  return R.pipe(R.juxt([_.getFirstMatch(_constants.ALIAS_REGEX), R.pipe(R.replace(_constants.ALIAS_REGEX, ''), R.replace(_constants.OPENING_PARAN_REGEX, ''), R.replace(_constants.CLOSING_PARAN_REGEX, ''), x => `[${x}]`, eval, R.map(evaluate))]), R.cond([[isPaletteAction, R.ifElse(R.propSatisfies(_.isSomething, 1), ([a, p]) => P[a](...p), ([a, p]) => P[a])], [isHelperAction, R.ifElse(R.propSatisfies(_.isSomething, 1), ([a, p]) => _[a](...p), ([a, p]) => _[a])], [R.T, R.identity]]))(action);
}
