...
function decodeStringyAction(action) {
  return R.pipe(R.juxt([_.getFirstMatch(ALIAS_REGEX), R.pipe(R.replace(ALIAS_REGEX, ''), R.replace(OPENING_PARAN_REGEX, ''), R.replace(CLOSING_PARAN_REGEX, ''), R.split(SEPARATOR_REGEX), R.without(['', undefined]), R.map(evaluate.bind(this)))]), R.ifElse(R.pipe(R.prop(0), R.has(R.__, P)), R.ifElse(R.pipe(R.prop(1), _.isSomething), ([a, p]) => P[a](...p), ([a, p]) => P[a]), R.identity))(action);
}
