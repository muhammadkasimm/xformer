...
const decodeObjectAction = exports.decodeObjectAction = R.curry(action => {
  ...
  return R.cond([[R.propSatisfies(R.has(R.__, P), 'name'), ({ name, params = [] }) => {
    return H.isSomething(params) ? P[name](...evaluateParams(params)) : P[name];
  }], [R.allPass([R.has('fn'), R.propSatisfies(H.typeMatches('function'), 'fn')]), ({ name, fn, params = [] }) => H.isSomething(params) ? fn(...evaluateParams(params)) : fn], [R.T, R.identity]])(action);
});
