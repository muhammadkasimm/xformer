...
function evaluate(str) {
  return R.pipe(R.tryCatch(eval, R.always(str)), R.when(R.test(/^[\$.]/), R.pipe(R.split('.'), R.path(R.__, this))), R.when(R.test(/^[X.]/), R.pipe(R.split('.'), R.last, decodeStringyAction)), R.when(R.both(_.typeMatches('object'), R.pipe(R.prop('name'), R.has(R.__, P))), decodeObjectAction))(str);
}
...
function decodeStringyAction(action) {
  return R.pipe(R.juxt([_.getFirstMatch(ALIAS_REGEX), R.pipe(R.replace(ALIAS_REGEX, ''), R.replace(OPENING_PARAN_REGEX, ''), R.replace(CLOSING_PARAN_REGEX, ''), x => `[${x}]`, eval, R.map(evaluate.bind(this)))]), R.ifElse(R.pipe(R.prop(0), R.has(R.__, P)), R.ifElse(R.propSatisfies(_.isSomething, 1), ([a, p]) => P[a](...p), ([a, p]) => P[a]), R.identity))(action);
}
...
function decodeAction(action) {
  try {
    return R.cond([[_.typeMatches('string'), decodeStringyAction.bind(this)], [_.typeMatches('object'), decodeObjectAction.bind(this)], [_.typeMatches('array'), R.pipe(decodePipe.bind(this), R.apply(R.pipe))]])(action);
  } catch (error) {
    return action;
  }
}
